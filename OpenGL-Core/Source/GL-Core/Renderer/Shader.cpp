...
void Shader::SetUniform1f(const std::string& i_Name, float v)
{
	GLCall(glUniform1f(GetUniformLocation(i_Name), v));
}
...
void Shader::SetUniformMat4f(const std::string& i_Name, const glm::mat4& matrix)
{
	GLCall(glUniformMatrix4fv(GetUniformLocation(i_Name), 1, GL_FALSE, &matrix[0][0]));
}
void Shader::SetUniform1iv(const std::string& i_Name, int count, int* value)
{
	GLCall(glUniform1iv(GetUniformLocation(i_Name), count, value));
}
